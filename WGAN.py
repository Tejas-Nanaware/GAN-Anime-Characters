...
def discriminator_model(image_shape=IMAGE_SHAPE):
    ...
    model.compile(optimizer=RMSprop(lr=0.0008, clipvalue=1.0, decay=1e-8), loss=wasserstein_loss, metrics=['accuracy'])
...
#         my_image = (predicted[i] * 255).astype(np.uint8) / 255
        plt.imshow(my_image)
    plt.savefig('./WGeneratedFigures/image_'+current_time+'.jpg', bbox_inches = 'tight', pad_inches = 0.1)
#     plt.show(block=True)
    plt.close('all')
...
for file in glob.glob('./WGeneratedFigures/*'):
    if file.endswith('.jpg'):
        os.remove(file)
for file in glob.glob('./WGANModels/*'):
    if file.endswith('.h5'):
        os.remove(file)
...
#         log.write('%d,%f,%f,%f,%f,%f,%f\n' % (step, real_disc_metrics[0], real_disc_metrics[1], gen_disc_metrics[0], gen_disc_metrics[1], gan_metrics[0], gan_metrics[1]))
    if ((step % 50) == 0):
        gan_model.save('./WGANModels/GANmodel_'+current_time+'.h5')
        gen_model.trainable = True
        gen_model.save('./WGANModels/GENmodel_'+current_time+'.h5')
        disc_model.trainable = True
        disc_model.save('./WGANModels/DISmodel_'+current_time+'.h5')
